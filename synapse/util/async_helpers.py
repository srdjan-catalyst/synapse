...
class ObservableDeferred(Generic[_T]):
    ...
    def __init__(self, deferred, consumeErrors=False):
        # type: (defer.Deferred[_T], bool) -> None
    ...
    def observe(self):
        ...
        """
        if not self._result:
            d = defer.Deferred()  # type: defer.Deferred[_T]
...
class ReadWriteLock:
    ...
    async def read(self, key: str) -> ContextManager:
        new_defer = defer.Deferred()  # type: defer.Deferred[None]
...
def timeout_deferred(deferred, timeout, reactor):
    # type: (defer.Deferred[_T], float, IReactorTime)->defer.Deferred[_T]
    ...
    """
    new_d = defer.Deferred()  # type: defer.Deferred[_T]
